...
static const std::unordered_map<std::string, std::string>& buildCommandMan() {
    static std::unordered_map<std::string, std::string> cmdMan = []() -> std::unordered_map<std::string, std::string> {
        ...
        m["plot"] =
            "plot[parameter=(speed|arrival_time_of_front);filename=<fname.png/jpg>;opt:range=(min,max);opt:cmap=<colormap>;opt:histbins=integer]\n"
            "Generates an image (PNG or JPG) of the simulation\n"
            "Example: plot[parameter=speed;filename=out.png;opt:range=(0,0.1);opt:cmap=viridis;opt:histbins=50]\n"
            "Arguments:\n"
            " - 'parameter': Simulation parameter to plot (speed|arrival_time_of_front|fuel)\n"
            " - 'filename': Output image file name\n"
            " - 'opt:range': Optional range for the plot (min,max)\n"
            " - 'opt:area = (8.36215875,41.711125,9.1366311,42.28667)\n"
            " - 'opt:cmap': Optional colormap (e.g., viridis)\n"
            " - 'opt:histbins': Optional flag to include a histogram with histbins bins between min and max";
