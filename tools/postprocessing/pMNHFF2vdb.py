...
try:
    ...
    pyopenvdb = None  # For illustration; ensure pyopenvdb is installed.
...
def createAnisotropicTransform(dx, dy, dz, xOrig, yOrig, zOrig):
    """
    Build a 4x4 matrix for anisotropic scaling + translation.
    """
    return pyopenvdb.createLinearTransform([
        [dx,   0.0,  0.0,  xOrig],
        [0.0,  dy,   0.0,  yOrig],
        [0.0,  0.0,  dz,   zOrig],
        [0.0,  0.0,  0.0,  1.0  ]
...
def gridToVDB(
    ...
    # Derive dx, dy, dz from the 1D arrays, or fallback if needed
    try:
        if len(x1D) > 1:
        ...
        # The origin is (x1D[0], y1D[0], zNew[0]) in world space
        transform = createAnisotropicTransform(
            dx, dy, dz,
            x1D[0], y1D[0], zNew[0]
        )
        grid.transform = transform

        print(f"[gridToVDB] Using dx={dx:.3f}, dy={dy:.3f}, dz={dz:.3f}, "
              f"origin=({x1D[0]:.3f},{y1D[0]:.3f},{zNew[0]:.3f}).")

    except Exception as e:
        print(f"[gridToVDB] transform creation failed: {e}. "
              f"Falling back to uniform voxelSize={fallbackVoxelSize}")
        grid.transform = pyopenvdb.createLinearTransform(fallbackVoxelSize)
    ...
    for i in range(nx):
        for j in range(ny):
            for kNew in range(newNz):
                ...
                if val_index < 0.0:
                    continue  # skip if below min altitude
                ...
                if val >= lower_threshold:
                    if val > upper_threshold:
                        val = upper_threshold
                    accessor.setValueOnly((i, j, kNew), float(val))
                    accessor.setActiveState((i, j, kNew), True)

    pyopenvdb.write(outname + ".vdb", grids=[grid])

    print(f"[gridToVDB] Wrote {outname}.vdb with final shape=({nx},{ny},{newNz}).")
    return grid

def ffmnhFileToVDB(inpattern="", pgdInfo="", outPath="", cleanFile=False,
    ...
    them out to .vdb using pyopenvdb.
    """
    # Derive file prefix
    fprefix = inpattern.split("/")[-1]

    # Grid key for domain (vertical coords, etc.)
    gridKey = "ZGRID"
    ...
    # Gather available variables by scanning .1.* files
    ...
    # If we only want to retrieve the number of time steps, do so and quit.
    if quitAfterCompute:
    ...
    for stepV in Allsteps:
        outname = "%s/%s.full.%d.vdb" % (outPath, fprefix, stepV)
    ...
    # Basic parse of PGD info (res, ni, nj, x0, y0)
    parts = pgdInfo.split(',')
    if len(parts) != 5:
    ...
    for inddom in range(numOfDomains):
        ...
        # Store shape boundaries (some attempts at "no overlap" indexing)
    ...
    # Construct X/Y arrays (though only used for reference in this example)
    ...
    # Reconstruct flow at mass points if needed
    midUVW = True
    x1D, y1D, zNew, zCoeffs =  genVoxelRemap(xxd, yyd, zzd, targetNz=200)

    print("REMM ",np.shape(zNew),np.shape(zCoeffs))
    appendedOutName = []   # list of "output.full.<step>.vdb" filenames
    stepsDone = []         # matching time/step values
    grids_to_write = []
    for stepV in selectedSteps:
        ...
        for indom in range(numOfDomains):
            ...
            feed_shape_x = nx + 1
            feed_shape_y = ny + 1
            # Note: i1, j1 not always used explicitly below.
        ...
        # If focusing on a single varName (e.g. "BRatio"), call our gridToVDB
        # with the chosen range. Example uses varmapAll[varName].
        if varName not in varmapAll:
            print("Variable '%s' not found for step %d." % (varName, stepV))
            continue
        print("DATMM ",np.shape(varmapAll[varName]))
        grid=gridToVDB(outname,varmapAll[varName],  x1D=x1D,    y1D=y1D, zNew=zNew,    zCoeffs=zCoeffs, rangeVdb=rangeVdb, fallbackVoxelSize=80.0)
        grid.name = f"{stepV}"
        grid["time"] = float(stepV)
        grids_to_write.append(grid)
        appendedOutName.append(outname + ".vdb")  # store actual filename with .vdb
        stepsDone.append(stepV)
        # If you want to store vectors or multiple grids, loop and call gridToVDB 
        # with different names or store them in one VDB file with multiple FloatGrids.
    saveJsonVdbIndex("%s/%sall.json" % (outPath, fprefix), appendedOutName, stepsDone)
    pyopenvdb.write("%s/%sall.vdb" % (outPath, fprefix), grids=grids_to_write)
...
if __name__ == "__main__":
    ...
    def main():
        parser = argparse.ArgumentParser(
            description=(
                "Convert ffmnh files to VDB format. Example:\n"
                "  python pMNHFF2VDB.py -fields MODEL1/output 80,69,69,-80,-80 vdbout1"
                " -varName=BRatio -rangeVdb=0.002,1.0 -steps 10 21\n"
            ),
            formatter_class=argparse.RawTextHelpFormatter
        )
        parser.add_argument('-fields', nargs=3,
                            help='Input: (1) mnhdump file pattern, (2) PGD info, (3) Output directory')
        parser.add_argument('-varName', help='Data variable to convert (e.g. BRatio)')
        parser.add_argument('-rangeVdb', help='Comma-separated lower,upper range for VDB storage', default="0.002,1.0")
        ...
        parser.add_argument('-norecompute', action='store_true',
                            help='Skip re-compute phase if VDB files exist')
